"""Rung 7's runner: fit the HSMM, decode onsets, write the explanation-quality artifact.

**Why this is not in ``explain/``, which is where it started.**
``tests/unit/test_explain_registry.py::test_the_explain_package_does_not_import_the_models_package``
walks every file under ``src/rakshak/explain/`` and refuses any import of
``rakshak.models``. That wall is not decoration: its sibling test refuses the reverse
direction, and between them they are what stops an explainer from acquiring a scoring path
by accident. The measurement half of Rung 7 genuinely needs ``rung7_hsmm.fit`` and
``seed_model`` — it fits the model — so it cannot live behind that wall, and the wall is
not the thing that should move.

So the file was split along the line the wall already draws:

* ``explain/hsmm_onset.py`` keeps the **explainer** — ``HsmmOnsetExplainer`` and
  ``first_change_point``. Both need a *decoder*, not the HSMM class, so they take one
  structurally (``rakshak.explain.hsmm_onset.Decoder``) and import nothing from
  ``models/``.
* This module keeps the **runner**, and does the concrete wiring: it imports the real
  ``HsmmNb``, fits it, and hands the fitted object to the explainer.

That is the same composition-root pattern ``cli.py`` already uses — it assembles ``Truth``
and hands the rungs plain arrays — and the same level ``score_rung5.py`` and
``score_rung6.py`` sit at. Neither the AST gate over ``models/`` and ``features/``
(``tests/gates/test_g4_no_leakage.py``) nor the explain-wall gate covers
``src/rakshak/*.py``, deliberately: the eval side has to be able to see both halves in
order to join them.

Everything below is the code that was in ``explain/hsmm_onset.py``, moved rather than
rewritten. No number it produces changes: the estimator, the fit pool, the pool RNG seed
(20260901) and the artifact schema are byte-for-byte what they were.
"""
...
from rakshak.explain.hsmm_onset import HsmmOnsetExplainer, first_change_point
from rakshak.explain.registry import ExplanationRequest, register, registered
from rakshak.models.rung7_hsmm import MAX_DURATION, STATE_NAMES, fit, seed_model

__all__ = ["EXPLANATION_DIR", "measure"]

ROOT: Final = Path(__file__).resolve().parents[2]
DATA_ROOT: Final = ROOT / "data" / "v2"

#: NOT ``data/v2/eval``. ``artifacts/build.py::read_result_rows`` globs that directory into
#: ``ladder.json``, so a Rung 7 file there *becomes* a ladder row whatever it says inside —
#: it would sit beside Rungs 0-6 with a blank PR-AUC and read as a rung that scored badly
#: rather than one that was never a scorer. #51 is explicit that Rung 7 runs at Stage 2
#: only, "never in the scoring path, never scored on PR-AUC", and this directory is what
#: enforces it.
...
def measure(
    *,
    ...
    for seed in seeds:
        ...
        explainer = HsmmOnsetExplainer(
            model=em.model, sequences=val_seqs, state_names=tuple(STATE_NAMES)
        )
...
def main() -> None:
    """``python -m rakshak.score_rung7``. ``rakshak.cli explain`` is the other door."""
    import argparse
