...
Lives here rather than in ``conftest.py`` because ``tests/gates/`` needs it too, and
``from conftest import ...`` only ever finds the conftest in the importing module's own
directory. ``tests/parity`` is on ``pythonpath`` so both suites can reach this by name.
